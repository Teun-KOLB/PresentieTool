...
function parseAndInit() {
    ...
    rows.forEach((row) => {
        ...
        if (dayMatch) {
            ...
            parsedRoster[currentDay], columnMapping = {};
...
function renderTable() {
    ...
    cohort.forEach(studentName => {
        ...
        else if (isInitiallyAbsent || isInitiallyGreyZero) { effectiveMinutes = 0 }
    ...
    let tableRows = students.map(student => {
        ...
        if (student.isAbsent || student.isGreyZero || student.isGreyKeep) { minuteClass += ' text-slate-300'; } 
        else if (student.isYellow) { minuteClass += ' text-yellow-600'; } 
        else { minuteClass += ' text-emerald-600'; }
        ...
        if (isSpecialSplit) {
            ...
            minuteClass = 'w-full';
        } else { minuteContent = student.effectiveMinutes + ' min'; }
        return `
